...
<template>
    <div class="space-y-6">
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        <tr v-for="metric in sortedMetrics" :key="metric.symbol"
                            class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    <span class="font-medium">{{ metric.symbol }}</span>
                                    <span v-if="metric.isInDrawdown"
                            ...
                            <td class="px-4 py-3">
                                <svg viewBox="0 0 100 30" class="w-24 h-8" preserveAspectRatio="none">
                                    ...
                                    <path v-for="(area, i) in generateDrawdownAreas(metric.equityHistory)" :key="i"
                                        :d="area.path" fill="currentColor" :opacity="area.opacity"
                                        class="text-red-500" />
                                    <!-- Equity curve -->
                                    <path :d="generateSparkline(metric.equityHistory)" fill="none" stroke="currentColor"
                                        stroke-width="1.5"
        ...
        <div class="lg:hidden space-y-4">
            <div v-for="metric in sortedMetrics" :key="metric.symbol"
                ...
                <div class="mb-3">
                    <svg viewBox="0 0 100 30" class="w-full h-16" preserveAspectRatio="none">
                        <!-- Drawdown shaded areas -->
                        <path v-for="(area, i) in generateDrawdownAreas(metric.equityHistory)" :key="i" :d="area.path"
                            fill="currentColor" :opacity="area.opacity" class="text-red-500" />
                        <!-- Equity curve -->
                        <path :d="generateSparkline(metric.equityHistory)" fill="none" stroke="currentColor"
                            stroke-width="1.5" :class="metric.isInDrawdown ? 'text-red-500' : 'text-blue-500'" />
                    </svg>
                </div>
